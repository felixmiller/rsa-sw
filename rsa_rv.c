...
/* Todo: We will provide an optimized assembly version of this function */
			 ...
			 const uint32_t *b)
{
	...
	uint64_t B = mula32(d0, key->n[0], (uint32_t)A);
	uint32_t i;
	for (i = 1; i < RSANUMWORDS; ++i) {
		A = mulaa32(a, b[i], c[i], A >> 32);
		B = mulaa32(d0, key->n[i], (uint32_t)A, B >> 32);
		c[i - 1] = (uint32_t)B;
	}
	A = (A >> 32) + (B >> 32);
	c[i - 1] = (uint32_t)A;
 ...
 */
static void mod_pow(const struct rsa_public_key *key,
         uint32_t *in,
         uint32_t *out,
				 uint32_t *workbuf32)
{
